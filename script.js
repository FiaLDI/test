					...
					if (!$(this).hasClass('copy-lead__button_disable')) {
						let pipeline_id = Number($('.copy-lead [name="select-pipeline"]').val()),
							...
							$.ajax({
								...
								dataType: 'json',
								beforeSend: function () {
									replaceTextSelectButton(pipeline_id, 'Загрузка...');
								},
								error: function () {
									replaceTextSelectButton(pipeline_id, 'Ошибка');
								},
								success: function (data) {
									replaceTextSelectButton(pipeline_id, 'Данные прочитаны');
						
									console.log('Полученные данные:', data);
						
									let contacts = data._embedded.contacts;
									let contact_ids = contacts.map(contact => contact.id);
						
									console.log('Контакты:', contacts);
									console.log('ID контактов:', contact_ids);
									...
									new_s = {
										...
										"_embedded": {
											"contacts": contact_ids.length > 0 ? [{ id: contact_ids[0] }] : [], // Максимум один контакт
											"tags": data._embedded.tags,
											"catalog_elements": data._embedded.catalog_elements,
											"loss_reason": data._embedded.loss_reason,
											"companies": data._embedded.companies,
										}
									};
						
									console.log('Сформированные данные для отправки:', new_s);
									...
									$.ajax({
										...
										success: function (response) {
											...
											replaceTextSelectButton(pipeline_id, 'Успешное создание');
										}
									});
								}
							});
